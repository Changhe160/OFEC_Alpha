...
struct proInfo {
	...
	std::string getFileName()const {
		...
		return "CSIWDN_" + nbnType + "_stage_" + std::to_string(m_stage) + "_aroundRadius_" + aourndRadiusName + "_initRadiusPrecision_" + std::to_string(m_precision);
	}
...
void runTotalTasks() {
	...
	auto saveDir = ofec::g_working_directory + "/cwidn_total3/";
...
void runTotalTasks2() {
	...
	GlobalInfo globalInfo;
	globalInfo.saveDir = saveDir;
	globalInfo.m_win_name = "_linux_10";
	//	globalInfo.m_win_name = "_win_local";
	std::list<proInfo>infos;
	...
	std::vector<int> initDis = { 1,2,3,4,5,6 };
	for (auto& curinit : initDis) {
		proInfo localInfo;
		localInfo.m_initType = ofec::CSIWDN::DistanceType::Continous;
		localInfo.m_precision = curinit;
		infos.push_back(localInfo);
	}
