...
void evaluateSolsSubTask2(std::vector<std::shared_ptr<ofec::SolutionBase>>& sols, int from, int to, ofec::Environment* env, ofec::Random* rnd) {
	auto pro = env->problem();
	ofec::Real pos = (pro->optimizeMode(0) == ofec::OptimizeMode::kMaximize) ? 1 : -1;
	...
	while (true) {
		...
		if (curIdx >= sols.size())break;
		sols[curIdx]->evaluate(env, false);
		sols[curIdx]->setFitness(pos * sols[curIdx]->objective(0));
	}
...
void evaluateSols(std::vector<std::shared_ptr<ofec::SolutionBase>>& sols, ofec::Environment* env, ofec::Random* rnd) {
	...
	std::cout << "evaluateSols num task\t" << num_task << std::endl;
...
void generateSolRasiu(std::shared_ptr<ofec::SolutionBase>& initSol, const ofec::SolutionBase& centerSol, double sampleRadius, ofec::Environment* env, ofec::Random* rnd) {
...
void nbnSolInt(const std::string& saveDir) {
	using namespace std;
	using namespace ofec;
	std::shared_ptr<ofec::Environment> env;
	genenrateCSWIDNenv(env);
	...
	auto cswidnPro = CAST_CSIWDN(env->problem());
	cswidnPro->setPhase(cswidnPro->totalPhase());
	cswidnPro->setDistanceType(ofec::CSIWDN::DistanceType::Integer);
	std::vector<std::shared_ptr<ofec::SolutionBase>> sols;
	auto& optBase = cswidnPro->optimaBase()->solutionBase(0);
	...
	std::string filename = "optSol_finalStage_IntSample";
	...
	calNBN_division(env.get(), subAreaIds, bestId, sols, nbnInfo.belong, nbnInfo.dis2parent);




	ofec::TreeGraphSimple nbn_graph;
	calculateNetwork(nbnInfo, nbn_graph, env.get(), rnd.get());
...
void nbnSolCon(const std::string& saveDir) {
	using namespace std;
	using namespace ofec;
	std::shared_ptr<ofec::Environment> env;
	genenrateCSWIDNenv(env);
	...
	auto cswidnPro = CAST_CSIWDN(env->problem());
	cswidnPro->setPhase(cswidnPro->totalPhase());
	cswidnPro->setDistanceType(ofec::CSIWDN::DistanceType::Continous);
	std::vector<std::shared_ptr<ofec::SolutionBase>> sols(1e6);
	auto& optBase = cswidnPro->optimaBase()->solutionBase(0);
	...
	std::string filename = "optSol_finalStage_ConSample";
...
void nbnSolConTest(const std::string& saveDir) {
	using namespace std;
	using namespace ofec;
	std::shared_ptr<ofec::Environment> env;
	genenrateCSWIDNenv(env);
...
void nbnSolConRadius(const std::string& saveDir) {
	using namespace std;
	using namespace ofec;
	std::shared_ptr<ofec::Environment> env;
	genenrateCSWIDNenv(env);
	...
	std::string filename = "optSol_finalStage_ConSample_r001";
...
void runTask() {
	...
	auto saveDir = ofec::g_working_directory + "/cwidn_win2/";
	std::filesystem::create_directories(saveDir);

	// generate soluton int
	time_t timer_start, timer_end;
	time(&timer_start);
	std::cout << "nbnSolInt" << std::endl;
...
void runTotalTasks() {
	...
	auto saveDir = ofec::g_working_directory + "/cwidn_winlocal/";
	std::filesystem::create_directories(saveDir);
	...
	for (auto& curphase : phases) {
		...
		generateSolutionsConRadius(optBase, curConRatio, sols, env.get(), rnd.get());
		...
		std::string numStr;
		{
			int number = curConRatio * 100;
			std::ostringstream oss;
			oss << std::setw(3) << std::setfill('0') << number; // 使用setw设置宽度，setfill填充0
			numStr = oss.str();
		}
		...
		ofec::ParameterVariantStream paramsStream;
		paramsStream << vFitness;
		auto filepath = saveDir + "optSol_stage" + to_string(curphase) + "_ConSample_r" + numStr + "_nbnfit.txt";
		outputToFile(paramsStream, filepath);
	...
	for (auto& curphase : phases) {
		...
		ofec::ParameterVariantStream paramsStream;
		paramsStream << vFitness;
		auto filepath = saveDir + "optSol_stage" + to_string(curphase) + "_IntSample" + "_nbnfit.txt";
		outputToFile(paramsStream, filepath);
	...
	std::vector<double> conRange = { 1.0, 0.25, 0.05, 0.01 };

	for (auto& curConRatio : conRange) {
		std::shared_ptr<ofec::Environment> env;
		genenrateCSWIDNenv(env);
		auto rnd = make_shared<Random>(0.5);
		auto cswidnPro = CAST_CSIWDN(env->problem());
		...
		std::shared_ptr<ofec::SolutionBase> initSol;
		generateSolRasiu(initSol, optBase, curConRatio, env.get(), rnd.get());
		std::vector<std::shared_ptr<ofec::SolutionBase>> sols(1e6);
		generateSolutionsInts(*initSol, sols, env.get(), rnd.get());
		...
		ofec::ParameterVariantStream paramsStream;
		paramsStream << vFitness;
		auto filepath = saveDir + "optSol_stage" + to_string(curphase) + "_IntSample_r" + numStr + "_nbnfit.txt";
		outputToFile(paramsStream, filepath);
