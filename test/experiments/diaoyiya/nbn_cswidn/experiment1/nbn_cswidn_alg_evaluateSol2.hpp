...
void evaluateFilterSolAndOutputInfo(const std::string& saveDir, const std::string& processName, TaskInfo& curTask) {
...
void evaluateAllAlg(const std::string& saveDir, const std::string& processName, int numRun)
{
	...
	for (int proId(0); proId < proPhases.size(); ++proId) {
		...
		for (auto& curphase : proPhases[proId]) {
			...
			evaluateFilterSolAndOutputInfo(saveDir, processName, curTask);
...
//
//
//	auto saveDir = ofec::g_working_directory + "/cswidn_algorithm_total_oneAlgorithm/";
//
...
namespace ofec {
	...
	void run(int argc, char* argv[]) {
		...
		auto saveDir = ofec::g_working_directory + "/cswidn_algorithm_runProcess_2_test/";
		...
		std::cout << "ver5 evaluateAllAlg\t" << avg << std::endl;
		evaluateAllAlg(saveDir, avg, 3);
