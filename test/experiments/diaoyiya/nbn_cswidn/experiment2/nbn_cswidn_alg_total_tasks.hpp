...
//std::vector<std::vector<ofec::AMP_GL_SaDE_Sampling::SolInfo>> copyptr_solInfos;
void evaluateProOnePhase(const std::string& saveDir,
	const std::string& saveDir2,
	const std::string& processName, int phaseTime, int numRun = 30) {
	...
	std::cout << "input solutions finished\t" << std::endl;
	...
	std::vector<int> mapSolId;
	std::vector<ofec::SolutionBase*>fileterSols;
	cswidnPro->fitlerSameSolutions(sols, fileterSols, mapSolId);

	for (auto& it : fileterSols) {
...

	{
		...
		paramsStream << fileterSols.size();
		for (auto& it : fileterSols) {
			pro->solutionToParameterVariants(*it, paramsStream);
		}
		ofec::variants_stream::outputToFile(paramsStream, saveDir2 + proname + "_phase_" + to_string(phaseTime) + "_solutions.txt");
	}

	{
		...
		paramsStream << mapSolId;
		ofec::variants_stream::outputToFile(paramsStream, saveDir2 + proname + "_phase_" + to_string(phaseTime) + "_mapSolId.txt");
...
void evaluateProOnePhaseMulti(const std::string& saveDir,
	const std::string& saveDir2,
	const std::string& processName, int numRun = 30) {
	std::string proname = "CSIWDN";
	for (int idx(25); idx <= 48; ++idx) {
		auto phaseTime = idx;
		auto filename = proname + "_phase_" + std::to_string(phaseTime) + "_solutions.txt";
		if (waitForFile(saveDir2, filename, processName)) {
			evaluateProOnePhase(saveDir, saveDir2, processName, phaseTime, numRun);
		}
...
void calNBNallTasks(const std::string& readdir, const std::string& saveDir) {
	using namespace ofec;
	using namespace ofec::utility;
	const std::string extension = "_solutions.txt";
	auto filenames = getFilesNamesWithExtension(readdir, extension);
	for (auto& it : filenames) {
		it = removeSuffix(it, extension);
	...
	for (auto& it : filenames) {
		...
		std::vector<std::shared_ptr<ofec::SolutionBase>> sols;
		{
			{
				ofec::ParameterVariantStream paramsStream;
				ofec::variants_stream::inputFromFile(paramsStream, readdir + curfilename + extension);
				size_t inputSize(0);
				paramsStream >> inputSize;
...
namespace ofec {
	...
	void run(int argc, char* argv[]) {
		...
		auto saveDir2 = ofec::g_working_directory + "/paper_cswidn/cswidn_algorithm_data_problemPhase/";
		...
		//	calNBNallTasks(saveDir2, networkDir);
