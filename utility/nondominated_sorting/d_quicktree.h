...
namespace NDS {
	...
	struct dt_node
	{
		std::pair<int, std::vector<double>> obj;
		std::vector<dt_node*> children;
		dt_node* parent = nullptr;
		int rank = -1;
		int ith; // the ith child of parents node
		dt_node(int lable, int n, double value) :obj(std::make_pair(lable, std::vector<double>(n, value))), children(1, nullptr) {}
		dt_node(std::pair<int, std::vector<double>>&& o) :obj(std::move(o)), children(1, nullptr) {}
		dt_node(const std::pair<int, std::vector<double>>& o) :obj(o), children(1, nullptr) {}
	};
	...
	public:
		...
		d_quicktree(int n) :m_root(new dt_node(-1, n, std::numeric_limits<double>::min())) {	}

		void insert(dt_node* &p, dt_node* q, std::pair<int, std::vector<double>> &obj) {

			if (p == nullptr) {
				dt_node* x = new dt_node(obj);
				m_nodes.push_back(std::unique_ptr<dt_node>(x));
				x->ith = 0;
				x->rank = q->rank + 1;
				x->parent = q;
				q->children[0] = x;
				...
				auto result = std::make_pair(dominationship::Dominated, 0);
				if (p != m_root) {
		...
		void _delete(const int index) {
			std::unique_ptr<dt_node>& todelete = m_nodes[index];
		...
		void build(std::vector< std::pair<int, std::vector<double>>>& data) {
			for (auto &i : data) {
		...
		void add(std::vector<double>& obj) {
			insert(m_root, nullptr, std::make_pair(int(m_nodes.size()), obj));
		}
	...
	private:
		std::vector<std::unique_ptr<dt_node>> m_nodes;
		dt_node *m_root;
