		...
		void sort() {
			...
			for (int k = 0; k < m_popsize; k++, ++i) {
				for (int j = 0; j<m_popsize; j++) {
					if (k != j) {
						auto compare_result = objective_compare(m_pop[j].second, m_pop[k].second, optimization_mode::Minimization);
						m_num += compare_result.second;
						if (compare_result.first == dominationship::Dominating) {//*m_pop[j]>*m_pop[k]
							rank_[k]++;
						}
						else if (compare_result.first == dominationship::Dominated) {//*m_pop[k]>*m_pop[j]
							(*i)[count[k]] = j;
							count[k]++;
						}
					}
