...
namespace OFEC {
	...
	void BBOB::initialize() {
		...
		if (m_name == "FUN_BBOB_F01_Sphere") {
			...
			std::vector<int> rperm(fmax(m_variable_size, NHIGHPEAKS21 - 1));
			...
			std::vector<double> fitvalues = { 1.1, 9.1 };
			m_condition_number = 1000.;
			std::vector<int> rperm(fmax(m_variable_size, NHIGHPEAKS22 - 1));
	...
	void BBOB::reshape(matrix &B, std::vector<real>& vector, int m, int n) {
		int i, j;
		for (i = 0; i < m; i++)
		{
			for (j = 0; j < n; j++)
	...
	void BBOB::computeRotation(matrix& rot, int Dim) {
