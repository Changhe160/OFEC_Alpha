...
namespace OFEC {
	namespace CEC2017 {
		C27::C27(param_map &v) :problem((v[param_proName]), (v[param_numDim]), 1), \
		...
		void C27::initialize() {
			...
			bool is_load = load_translation("instance/problem/continuous/constrained/CEC2717/data/");  //data path
			if (!is_load) {
			...
			load_rotation("instance/problem/continuous/constrained/CEC2717/data/");
			set_global_opt(m_translation.data());
		}
		void C27::evaluate__(real *x, std::vector<real>& obj, double & cons) {
			...
			if (m_translation_flag) {
				translate(x_ori.data());
				translate_origin(x_ori.data());
				double temp = 0.;
