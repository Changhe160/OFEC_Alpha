 ...
 * */
{
  ...
  if (!t->sp.flgsupplemented) {
    ...
    if (!isNoneStr(t->sp.filename)) /* TODO: should this be done in readpara_SupplementDefaults? */
      cmaes_readpara_WriteToFile(&t->sp, ((std::string)OFEC::global::ms_arg.at("workingDir") + "result/actparcmaes.par").c_str());
  }
                          ...
                          char const *s3, char const *s4)
{
  ...
  */
  time_t t = time(NULL);
  FILE *fp = fopen(((std::string)OFEC::global::ms_arg.at("workingDir") + "result/errcmaes.err").c_str(), "a");
  if (!fp)
    {
      printf("\nFATAL ERROR: %s\n", s2 ? szCat(s1, s2, s3, s4) : s1);
