...
namespace OFEC {
	NSDE::NSDE(param_map &v) :DEpopulation(v[param_popSize], v[param_numDim]), m_order_list(v[param_popSize]), \
		m_m(9), m_dis((v[param_popSize] / (m_m + 1)) + 1), m_seed((v[param_popSize] / (m_m + 1)) + 1) \
	{
		setMutationStrategy(DE_rand_1);
		setParmeter(0.7, 0.5);  // set CR=0.7, F=0.5
	}


	void NSDE::selectSubpop() {
		int i = 0;
		sort();
