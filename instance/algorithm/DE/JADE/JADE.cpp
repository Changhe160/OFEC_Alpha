...
namespace OFEC {
	JADE::JADE(param_map &v) :DEpopulation<JADEindi>(v[param_popSize],v[param_numDim]), m_candi(3, nullptr), m_pcentBestIdx(size()), \
		m_pcentBest(size()), mv_F(size()), mv_CR(size()) {
	...
	void JADE::selectTrial(int base) {
	...
	evaluation_tag JADE::run_() {
		evaluation_tag tag = evaluation_tag::Normal;
		CONTINOUS_CAST->set_objective_monitor_true();
		...
		while (tag != evaluation_tag::Terminate) {
			...
			update_best();
			double error = fabs(m_best[0]->get_objective()[0] - gopt[0]);
